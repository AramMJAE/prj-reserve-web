...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (supabase) {
      ...
      setLoading(false);
      if (error) {
        showToast(error.message === "Invalid login credentials" ? "이메일 또는 비밀번호가 올바르지 않습니다" : error.message, "error");
        return;
      }
      if (data.user) {
      ...
      // Supabase 미연동 시 mock login
      setTimeout(() => {
        const isAdmin = email === "admin@staylog.kr";
        setUser({ id: "user-01", email, name: email.split("@")[0], role: isAdmin ? "admin" : "user" });
        showToast("로그인되었습니다", "success");
        setLoading(false);
        router.push("/");
      }, 800);
  ...
  const handleKakaoLogin = async () => {
    if (!supabase) {
      showToast("소셜 로그인은 Supabase 연동 후 사용 가능합니다", "info");
      return;
    }
